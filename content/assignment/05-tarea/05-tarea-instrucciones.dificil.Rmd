...
datos <- haven::read_sav(file ="bases/procesadas/Latrille.sav")
...
datos <- datos %>% filter(entrevistado == 1) %>% 
    ...
    mutate_at(vars(edad, trabaja, riesgo_fct, cuidarse_dummy), funs(forcats::as_factor(.)))
...
```{r pred-table}
get_model_data(model2, type = c("pred"))$trabaja
```
...
model0 <- svyglm(cuidarse_dummy ~ 1, family = binomial(link = "logit"), design = movid_design, data = datos)

model3 <- svyglm(cuidarse_dummy ~ edad, family = binomial(link = "logit"), design = movid_design, data = datos)

model4 <- svyglm(cuidarse_dummy ~ trabaja, family = binomial(link = "logit"), design = movid_design, data = datos)

model5 <- svyglm(cuidarse_dummy ~ edad + trabaja, family = binomial(link = "logit"), design = movid_design, data = datos)

model6 <- svyglm(cuidarse_dummy ~riesgo_fct + edad +  trabaja, family = binomial(link = "logit"), design = movid_design, data = datos)
...
d. Realiza un gráfico de valores predichos de la actitud hacia los sindicatos según sexo [9 puntos]
